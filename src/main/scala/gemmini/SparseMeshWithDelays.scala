...
class SparseMeshWithDelays[T <: Data: Arithmetic, U <: TagQueueTag with Data]
  ...
  val io = IO(new Bundle {
    ...
    //val forwarded = Output(Vec(meshColumns, Vec(tileColumns, outputType)))
  })
  ...
  val mesh = Module(new SparseMesh(inputType, outputType, accType, df, tree_reduction, tile_latency, max_simultaneous_matmuls, output_delay, tileRows, tileColumns, meshRows, meshColumns))
  ...
  val mesh_out = shifted(Mux(mesh.io.out_control(0)(0).dataflow === Dataflow.OS.id.U, mesh.io.out_c, mesh.io.out_b), outBanks, true)
  ...
  val forwarded = shifted(mesh.io.out_forwarded_b, outBanks, true)
  val forwardValid = shifted(mesh.io.out_forwarded_b_valid, outBanks, true)(0)(0)
  //io.forwarded := forwarded

  class ForwardData extends Bundle{
    val data = C_TYPE.cloneType
    val id = UInt(log2Up(max_simultaneous_matmuls).W)

    // def make_this_garbage(dummy: Int=0): Unit = {
    //   data := VecInit(Seq.fill(meshColumns)(VecInit(Seq.fill(tileColumns)(0.U.asTypeOf(inputType)))))
    // }
  }


  //cols or rows?
  val forwardq = Module(new Queue(new ForwardData, entries = meshRows))
  forwardq.io.enq.valid := forwardValid
  forwardq.io.enq.bits.data := forwarded
  forwardq.io.enq.bits.id := matmul_id

  val forward_ready = mesh_out(0)(0) === 0.U.asTypeOf(outputType)

  val forwardq_ready = io.resp.valid && out_matmul_id === forwardq.io.deq.bits.id && forward_ready
  forwardq.io.deq.ready := forwardq_ready

  val zero = VecInit(Seq.fill(meshColumns)(VecInit(Seq.fill(tileColumns)(0.U.asTypeOf(inputType)))))

  val forward_data = Mux(forwardq.io.deq.valid, forwardq.io.deq.bits.data, zero)

  io.resp.bits.data := Mux(forward_ready === 1.U && forwardq_ready === 1.U, forward_data, VecInit(mesh_out))

  //io.forwarded := forward_data
