...
// }
  ...
  //   meshT(c).foldLeft((io.in_b(c), io.in_valid(c))) {
  //     case ((in_b, valid), tile) =>
  //     // Forward the data directly if the condition is met, otherwise pipeline
  //     tile.io.in_b := pipeWithForwarding(valid.head, in_b.bits, tile_latency+1, forwardingCondition = in_b(0).forward, forwardedData = tile.io.out_b)
  //     (tile.io.out_b, tile.io.out_valid)
